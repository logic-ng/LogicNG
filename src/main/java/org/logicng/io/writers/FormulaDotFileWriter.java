...
/**
 * A dot file writer for a formula. Writes the internal data structure of the
 * formula to a dot file.
 * @version 2.4.0
 * @since 1.0
 ...
 *             {@link FormulaDagGraphicalGenerator} or
 *             {@link FormulaAstGraphicalGenerator} within the new graphical
 *             writer framework.
 */
@Deprecated
public final class FormulaDotFileWriter {
    ...
    /**
     * Writes a given formula's internal data structure as a dot file with the
     * default style configuration.
     * @param fileName      the file name of the dot file to write, will be
     *                      extended by suffix {@code .dot} if not already
     *                      present
     * @param formula       the formula
     * @param alignLiterals indicates whether all literals should be aligned at
     *                      the same vertical level
     * @throws IOException if there was a problem writing the file
     */
    public static void write(final String fileName, final Formula formula, final boolean alignLiterals)
            throws IOException {
    ...
    public static void write(final File file, final Formula formula, final boolean alignLiterals) throws IOException {
        FormulaDagGraphicalGenerator.builder().alignTerminals(alignLiterals).build().translate(formula).write(file,
                GraphicalDotWriter.get());
    }
}
