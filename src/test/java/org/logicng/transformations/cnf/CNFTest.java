...
public class CNFTest {
  ...
  public void testNot() throws ParserException {
    ...
    PropositionalParser p = new PropositionalParser(F.f);
    Assert.assertEquals(p.parse("~a"), p.parse("~a").transform(cnf));
    Assert.assertEquals(p.parse("a"), p.parse("~~a").transform(cnf));
    Assert.assertEquals(p.parse("a & ~b"), p.parse("~(a => b)").transform(cnf));
    Assert.assertEquals(p.parse("~a & ~b & (x | y)"), p.parse("~(~(a | b) => ~(x | y))").transform(cnf));
    Assert.assertEquals(p.parse("(~a | ~b) & (a | b)"), p.parse("~(a <=> b)").transform(cnf));
    Assert.assertEquals(p.parse("(a | b | x | y) & (~a | ~x) & (~a | ~y) & (~b | ~x) & (~b | ~y)"), p.parse("~(~(a | b) <=> ~(x | y))").transform(cnf2));
    Assert.assertEquals(p.parse("~a | ~b | x | y"), p.parse("~(a & b & ~x & ~y)").transform(cnf));
    Assert.assertEquals(p.parse("~a & ~b & x & y"), p.parse("~(a | b | ~x | ~y)").transform(cnf));
    Assert.assertEquals(p.parse("~a & ~b & x & y"), p.parse("~(a | b | ~x | ~y)").transform(cnf));
