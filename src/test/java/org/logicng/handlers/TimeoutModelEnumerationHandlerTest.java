...
class TimeoutModelEnumerationHandlerTest {
    ...
    public void testFoundModel() throws InterruptedException {
        final TimeoutModelEnumerationHandler handler =
                new TimeoutModelEnumerationHandler(100, TimeoutHandler.TimerType.SINGLE_TIMEOUT);
        handler.started();
    ...
    public void testThatMethodsAreCalled() throws ParserException {
        ...
        for (final SATSolver solver : this.solvers) {
            ...
            final ModelEnumerationFunction me =
                    ModelEnumerationFunction.builder().handler(handler).variables(formula.variables()).build();
    ...
    public void testTimeoutHandlerSingleTimeout() {
        ...
        for (final SATSolver solver : this.solvers) {
            ...
            assertThat(result).isNotNull(); // assignments found so far are
                                            // returned
        }
    }
    ...
    public void testTimeoutHandlerFixedEnd() {
        ...
        for (final SATSolver solver : this.solvers) {
            solver.add(formula);
            final TimeoutModelEnumerationHandler handler = new TimeoutModelEnumerationHandler(
                    System.currentTimeMillis() + 100L, TimeoutHandler.TimerType.FIXED_END);
            final ModelEnumerationFunction me =
                    ModelEnumerationFunction.builder().handler(handler).variables(formula.variables()).build();
