...
package org.logicng.formulas.extendedFormulaFactory.transformations.cnf;
...
public class CNFEFTest {
  ...
  public void testNot() throws ParserException {
    ...
    Assert.assertEquals(p.parse("~a2 & ~b2 & x2 & y2"), p.parse("~(a2 | b2 | ~x2 | ~y2)").transform(cnf));
    Assert.assertEquals(p.parse("~a2 & ~b2 & x2 & y2"), p.parse("~(a2 | b2 | ~x2 | ~y2)").transform(cnf));
    Assert.assertEquals(10, handler2.distCount);
    Assert.assertEquals(7, handler2.clauseCount);
    Assert.assertEquals(4, handler2.longestClause);
  }
